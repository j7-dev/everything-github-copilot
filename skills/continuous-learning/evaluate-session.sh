...
# Runs on Stop hook to extract reusable patterns from Copilot CLI sessions
#
...
# Extracted skills saved to: ~/.copilot/skills/learned/

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_FILE="$SCRIPT_DIR/config.json"
LEARNED_SKILLS_PATH="${HOME}/.copilot/skills/learned"
...
if [ -f "$CONFIG_FILE" ]; then
  if ! command -v jq &>/dev/null; then
    ...
    LEARNED_SKILLS_PATH=$(jq -r '.learned_skills_path // "~/.copilot/skills/learned/"' "$CONFIG_FILE" | sed "s|~|$HOME|")
  fi
fi
...
# Get transcript path from stdin JSON (Copilot CLI hook input)
...
if [ -z "$transcript_path" ]; then
  transcript_path="${COPILOT_TRANSCRIPT_PATH:-}"
fi
