---
name: configure-ecc
description: Interactive installer for Everything Copilot CLI — guides users through selecting and installing skills and rules to user-level or project-level directories, verifies paths, and optionally optimizes installed files.
origin: ECC
---

# Configure Everything Copilot CLI (ECC)

An interactive, step-by-step installation wizard for the Everything Copilot CLI project. Uses `AskUserQuestion` to guide users through selective installation of skills and rules, then verifies correctness and offers optimization.

## When to Activate

- User says "configure ecc", "install ecc", "setup everything copilot cli", or similar
...
This skill must be accessible to Copilot CLI before activation. Two ways to bootstrap:
1. **Via Plugin**: `/plugin install everything-copilot-cli` — the plugin loads this skill automatically
2. **Manual**: Copy only this skill to `~/.copilot/skills/configure-ecc/SKILL.md`, then activate by saying "configure ecc"
...
```bash
rm -rf /tmp/everything-copilot-cli
git clone https://github.com/affaan-m/everything-copilot-cli.git /tmp/everything-copilot-cli
```

Set `ECC_ROOT=/tmp/everything-copilot-cli` as the source for all subsequent copy operations.
...
Options:
  - "User-level (~/.copilot/)" — "Applies to all your Copilot CLI projects"
  - "Project-level (.copilot/)" — "Applies only to the current project"
  - "Both" — "Common/shared items user-level, project-specific items project-level"
```

Store the choice as `INSTALL_LEVEL`. Set the target directory:
- User-level: `TARGET=~/.copilot`
- Project-level: `TARGET=.copilot` (relative to current project root)
...
```bash
grep -rn "~/.copilot/" $TARGET/skills/ $TARGET/rules/
grep -rn "../common/" $TARGET/rules/
grep -rn "skills/" $TARGET/skills/
```

**For project-level installs**, flag any references to `~/.copilot/` paths:
- If a skill references `~/.copilot/settings.json` — this is usually fine (settings are always user-level)
- If a skill references `~/.copilot/skills/` or `~/.copilot/rules/` — this may be broken if installed only at project level
...
- `continuous-learning-v2` references `~/.copilot/homunculus/` directory
...
3. **Issue**: What's wrong (e.g., "references ~/.copilot/skills/python-patterns but python-patterns was not installed")
4. **Suggested fix**: What to do (e.g., "install python-patterns skill" or "update path to .copilot/skills/")
...
### "Skills not being picked up by Copilot CLI"
- Verify the skill directory contains a `SKILL.md` file (not just loose .md files)
- For user-level: check `~/.copilot/skills/<skill-name>/SKILL.md` exists
- For project-level: check `.copilot/skills/<skill-name>/SKILL.md` exists
...
- Restart Copilot CLI after installing rules

### "Path reference errors after project-level install"
- Some skills assume `~/.copilot/` paths. Run Step 4 verification to find and fix these.
- For `continuous-learning-v2`, the `~/.copilot/homunculus/` directory is always user-level — this is expected and not an error.
