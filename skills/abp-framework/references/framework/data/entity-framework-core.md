...
> 來源：https://abp.io/docs/9.3/framework/data/entity-framework-core
...
( BookType is a simple enum here and not important) And you want to create a new Book entity in a domain service :
...
You generally want to derive from the IRepository to inherit standard repository methods (while, you don't have to do). Repository interfaces are defined in the domain layer of a layered application. They are implemented in the data/infrastructure layer ( EntityFrameworkCore project in a startup template ).
...
> 
AsyncExecuter is used to execute async LINQ extensions without depending on the EF Core. If you add EF Core NuGet package reference to your project, then you can directly use await query.FirstOrDefaultAsync() . But, this time you depend on the EF Core in your domain layer. See the repository document to learn more.
...
Some of the standard Repository methods have optional includeDetails parameters;
...
1. Install the Microsoft.EntityFrameworkCore.Proxies package into your project (typically to the EF Core integration project)
...
1. Make your navigation properties and collections virtual . Examples:
...
ABP provides read-only repository interfaces ( IReadOnlyRepository<...> or IReadOnlyBasicRepository<...> ) to explicitly indicate that your purpose is to query data, but not change it. If so, you can inject these interfaces into your services.
...
> 
The Object Extension feature need the Change Tracking , which means you can't use the read-only repositories for the entities that have extension properties(MapEfCoreProperty) , Please see the Repositories documentation to learn the change tracking behavior.
...
See the EF Core Shadow and Indexer Properties document for more information.
