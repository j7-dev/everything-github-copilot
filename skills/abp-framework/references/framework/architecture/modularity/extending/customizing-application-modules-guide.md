...
> 來源：https://abp.io/docs/9.3/framework/architecture/modularity/extending/customizing-application-modules-guide

# Customizing the Existing Modules

ABP has been designed to support to build fully modular applications and systems. It also provides some pre-built application modules those are ready to use in any kind of application.
...
In this case, you typically add the source code of the module to your solution and replace every package reference in the solution with its corresponding local project references. ABP CLI 's add-module command automates this process for you with the --with-source-code parameter. This command can also replace a module by its source code if the module already installed as NuGet packages.
...
If you only need to get/set extra data on an existing entity, follow the Extending Entities document.
...
- Entity Action Extensions for Angular
...
- Data Table Column Extensions for Angular
...
- Page Toolbar Extensions for Angular

#### Others

- Dynamic Form Extensions for Angular

## See Also

Also, see the following documents:

- See the localization document to learn how to extend existing localization resources.
- See the settings document to learn how to change setting definitions of a depended module.
- See the authorization document to learn how to change permission definitions of a depended module.
