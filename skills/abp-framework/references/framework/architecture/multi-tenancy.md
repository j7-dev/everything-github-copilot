...
> 來源：https://abp.io/docs/9.3/framework/architecture/multi-tenancy
...
> 
Multi-Tenancy is disabled in the ABP by default. However, it is enabled by default when you create a new solution using the startup template . MultiTenancyConsts class in the solution has a constant to control it in a single place.
...
Saas module (PRO) allows you to set a connection string for any tenant (as optional), so you can achieve any of the approaches.

> 
You can see the community article Multi-Tenancy with Separate Databases in .NET and ABP Framework for more details about different database architectures with practical implementation details.
...
IMultiTenant interface just defines a TenantId property. When you implement this interface, ABP automatically filters entities for the current tenant when you query from database. So, you don't need to manually add TenantId condition while performing queries. A tenant can not access to data of another tenant by default.
...
As mentioned before, ABP handles data isolation between tenants using the Data Filtering system. In some cases, you may want to disable it and perform a query on all the data, without filtering for the current tenant.
...
> 
There is an example that uses the subdomain to determine the current tenant.
...
The tenant management module is included in the startup templates and implements the ITenantStore interface to get the tenants and their configuration from a database. It also provides the necessary functionality and UI to manage the tenants.
...
The Tenant Management module provides a basic UI to manage your tenants. It is pre-configured for the application startup template .

### A note about separate database per tenant approach in open source version

While ABP fully supports this option, managing connection strings of tenants from the UI is not available in open source version. You need to have Saas module (PRO) .
...
- Features
- Article: Multi-Tenancy with Separate Databases in .NET and ABP Framework
