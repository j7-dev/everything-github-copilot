...
> 來源：https://abp.io/docs/9.3/framework/infrastructure/text-templating/razor
...
1. Add the Volo.Abp.TextTemplating.Razor NuGet package to your project:
...
1. Add the AbpTextTemplatingRazorModule to the dependency list of your module:
...
WithVirtualFilePath indicates that we are using the Virtual File System to store the template content. Create a Hello.cshtml file inside your project and mark it as " embedded resource " on the properties window:
...
- RenderAsync gets two fundamental parameters: templateName : The name of the template to be rendered ( Hello in this example). model : An object that is used as the model inside the template (a HelloModel object in this example).
...
- The source code of the sample application developed and referred through this document.
- Localization system .
- Virtual File System .
