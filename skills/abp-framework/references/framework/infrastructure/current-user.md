...
> 來源：https://abp.io/docs/9.3/framework/infrastructure/current-user
...
- TenantId (Guid?): Tenant Id of the current user, which can be useful for a multi-tenant application. Returns null , if the current user is not assigned to a tenant.
...
- Default values for the UserName , UserId , Role and Email properties are set from the System.Security.Claims.ClaimTypes class, but you can change them.
