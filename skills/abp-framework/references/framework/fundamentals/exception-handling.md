...
> 來源：https://abp.io/docs/9.3/framework/fundamentals/exception-handling
...
- For the AbpAuthorizationException : Returns 401 (unauthorized) if user has not logged in. Returns 403 (forbidden) if user has logged in.
...
It is possible to be informed when the ABP handles an exception . It automatically logs all the exceptions to the standard logger , but you may want to do more.
