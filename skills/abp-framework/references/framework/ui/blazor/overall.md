...
> 來源：https://abp.io/docs/9.3/framework/ui/blazor/overall

# Blazor UI: Overall

Blazor is a framework for building interactive client-side web UI with .NET. It enables .NET developers to create Single-Page Web Applications using C# and the Razor syntax.
...
- Get started with the Blazor UI for the ABP.
- Web Application Development Tutorial with the Blazor UI.
...
Dynamic C# Client Proxy system makes extremely easy to consume server side HTTP APIs from the UI. You just inject the application service interface and consume the remote APIs just like using local service method calls.
...
See the Dynamic C# Client Proxies document for more.
...
- Blazorise as a component library that supports the Bootstrap and adds extra components like Data Grid and Tree.
- FontAwesome as the fundamental CSS font library.
- Flag Icon as a library to show flags of countries.
...
> 
Beginning from June, 2021, the Blazorise library has dual licenses; open source & commercial. Based on your yearly revenue, you may need to buy a commercial license. See this post to learn more. The Blazorise license is bundled with ABP and commercial customers doesn't need to buy an extra Blazorise license.

### The Layout

The themes provide the layout. So, you have a responsive layout with the standard features already implemented. The screenshot below has taken from the layout of the Basic Theme :

See the Theming document for more layout options and other details.

### Layout Parts

A typical layout consists of multiple parts. The Theming system provides menus , toolbars , page alerts and more to dynamically control the layout by your application and the modules you are using.
...
See the Managing Global Scripts & Styles document.

## Services

ABP provides useful services that you can consume in your applications. Some of them are;

- IUiMessageService is used to show modal messages to the user.
- IUiNotificationService is used to show toast-style notifications.
- IAlertManager is used to show in-page alerts.
- ISettingProvider is used to access to the current setting values.
- ICurrentUser and ICurrentTenant is used to get information about the current user and the tenant.

## Dependency Injection

Razor components doesn't support constructor injection by default. ABP makes possible to inject dependencies into the constructor of the code-behind file of a component.
...
ABP makes this possible by auto registering components to and resolving the component from the Dependency Injection system.
...
While the theme and some modules come as NuGet packages, you can still replace/override and customize them on need. See the Customization / Overriding Components document.
