...
> 來源：https://abp.io/docs/9.3/framework/ui/mvc-razor-pages/forms-validation
...
- ABP Form tag helpers ( abp-input , abp-select , abp-radio ...) render a single form element with handling localization and client side validation.
...
> 
This document is for the client side validation and it doesn't cover the server side validation. Check the validation document for server side validation infrastructure.

## The Classic Way

In a typical Bootstrap based ASP.NET Core MVC / Razor Pages UI, you need to write such a boilerplate code to create a simple form element:
...
> 
See its own document for all options of the abp-dynamic-form tag helper.
...
> 
See the ABP Form Tag Helpers document for details of these tag helpers and their options.
...
ABP uses the property name as the field name on the user interface. You typically want to localize this name based on the current culture.
