...
> 來源：https://abp.io/docs/9.3/framework/ui/mvc-razor-pages/javascript-api/ajax
...
- Automatically handles & localize the errors and informs the user (using the abp.message ). So you typically don't care about errors.
...
> 
While abp.ajax makes the AJAX call pretty easier, you typically will use the Dynamic JavaScript Client Proxy system to perform calls to your server side HTTP APIs. abp.ajax can be used when you need to perform low level AJAX operations.
...
- abp.ajax.logError function logs errors using the abp.log.error(...) by default.
- abp.ajax.showError function shows the error message using the abp.message.error(...) by default.
