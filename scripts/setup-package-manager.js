...
function showHelp() {
  console.log(`
Package Manager Setup for Copilot CLI
...
Options:
  ...
  --global <pm>   Set global preference (saves to ~/.copilot/package-manager.json)
  --project <pm>  Set project preference (saves to .copilot/package-manager.json)
...
function detectAndShow() {
  ...
  console.log(`  Environment var: ${process.env.COPILOT_PACKAGE_MANAGER || 'not set'}`);
  console.log('');
...
function setGlobal(pmName) {
  ...
  try {
    ...
    console.log('  Saved to: ~/.copilot/package-manager.json');
    console.log('');
  } catch (err) {
