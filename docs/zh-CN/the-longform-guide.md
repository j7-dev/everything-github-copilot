# 关于 Copilot CLI 的完整长篇指南

![Header: The Longform Guide to Everything Copilot CLI](../../assets/images/longform/01-header.png)

***

> **前提**：本指南建立在 [关于 Copilot CLI 的简明指南](./the-shortform-guide.md) 之上。如果你还没有设置技能、钩子、子代理、MCP 和插件，请先阅读该指南。

![Reference to Shorthand Guide](../../assets/images/longform/02-shortform-reference.png)
*速记指南 - 请先阅读它*

在简明指南中，我介绍了基础设置：技能和命令、钩子、子代理、MCP、插件，以及构成有效 Copilot CLI 工作流骨干的配置模式。那是设置指南和基础架构。
...
简明指南和长篇指南中涵盖的所有内容都可以在 GitHub 上找到：`github.com/affaan-m/everything-copilot-cli`
...
*会话存储示例 -> https://github.com/affaan-m/everything-copilot-cli/tree/main/examples/sessions*
...
一旦你制定了计划并清除了上下文（Copilot CLI 中计划模式的默认选项），你就可以根据计划工作。当你积累了大量与执行不再相关的探索性上下文时，这很有用。对于策略性压缩，请禁用自动压缩。在逻辑间隔手动压缩，或创建一个为你执行此操作的技能。

**高级：动态系统提示注入**

我学到的一个模式是：与其将所有内容都放在 CLAUDE.md（用户作用域）或 `.copilot/rules/`（项目作用域）中，让它们每次会话都加载，不如使用 CLI 标志动态注入上下文。
...
```bash
# Daily development
alias claude-dev='claude --system-prompt "$(cat ~/.copilot/contexts/dev.md)"'

# PR review mode
alias claude-review='claude --system-prompt "$(cat ~/.copilot/contexts/review.md)"'

# Research/exploration mode
alias claude-research='claude --system-prompt "$(cat ~/.copilot/contexts/research.md)"'
```
...
我已经构建了这些钩子，它们位于仓库的 `github.com/affaan-m/everything-copilot-cli/tree/main/hooks/memory-persistence`
...
**解决方案：** 当 Copilot CLI 发现一些不平凡的事情时——调试技巧、变通方法、某些项目特定的模式——它会将该知识保存为一个新技能。下次出现类似问题时，该技能会自动加载。

我构建了一个实现此功能的持续学习技能：`github.com/affaan-m/everything-copilot-cli/tree/main/skills/continuous-learning`
...
*在我们的 50 项任务基准测试中，mgrep + Copilot CLI 使用了比基于 grep 的工作流少约 2 倍的 token，且判断质量相似或更好。来源：https://github.com/mixedbread-ai/mgrep*
...
当运行多个 Copilot CLI 实例时，使用“级联”模式进行组织：
...
另请参阅：ccstatusline（用于自定义 Copilot CLI 状态栏的社区项目）

### 语音转录

用你的声音与 Copilot CLI 对话。对很多人来说比打字更快。
...
* [YK: 32 个 Copilot CLI 技巧](https://agenticcoding.substack.com/p/32-copilot-cli-tips-from-basics-to)
...
*两份指南中涵盖的所有内容都可以在 GitHub 上的 [everything-copilot-cli](https://github.com/affaan-m/everything-copilot-cli) 找到*
