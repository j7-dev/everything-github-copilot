---
name: configure-ecc
description: Everything Copilot CLI 的交互式安装程序 — 引导用户选择并安装技能和规则到用户级或项目级目录，验证路径，并可选择性地优化已安装的文件。
---

# 配置 Everything Copilot CLI (ECC)

一个交互式、分步安装向导，用于 Everything Copilot CLI 项目。使用 `AskUserQuestion` 引导用户选择性安装技能和规则，然后验证正确性并提供优化。

## 何时激活

* 用户说 "configure ecc"、"install ecc"、"setup everything copilot cli" 或类似表述
* 用户想要从此项目中选择性安装技能或规则
* 用户想要验证或修复现有的 ECC 安装
* 用户想要为其项目优化已安装的技能或规则
...
此技能必须在激活前对 Copilot CLI 可访问。有两种引导方式：

1. **通过插件**: `/plugin install everything-copilot-cli` — 插件会自动加载此技能
2. **手动**: 仅将此技能复制到 `~/.copilot/skills/configure-ecc/SKILL.md`，然后通过说 "configure ecc" 激活
...
将 `ECC_ROOT=/tmp/everything-copilot-cli` 设置为所有后续复制操作的源。
...
**对于项目级别安装**，标记任何对 `~/.copilot/` 路径的引用：

* 如果技能引用 `~/.copilot/settings.json` — 这通常没问题（设置始终是用户级别的）
* 如果技能引用 `~/.copilot/skills/` 或 `~/.copilot/rules/` — 如果仅安装在项目级别，这可能损坏
...
* `continuous-learning-v2` 引用 `~/.copilot/homunculus/` 目录
...
1. **文件**：包含问题引用的文件
2. **行号**：行号
3. **问题**：哪里出错了（例如，"引用了 ~/.copilot/skills/python-patterns 但 python-patterns 未安装"）
4. **建议的修复**：该怎么做（例如，"安装 python-patterns 技能" 或 "将路径更新为 .copilot/skills/"）
...
* 对于用户级别：检查 `~/.copilot/skills/<skill-name>/SKILL.md` 是否存在
* 对于项目级别：检查 `.copilot/skills/<skill-name>/SKILL.md` 是否存在
...
* 有些技能假设 `~/.copilot/` 路径。运行步骤 4 验证来查找并修复这些问题。
* 对于 `continuous-learning-v2`，`~/.copilot/homunculus/` 目录始终是用户级别的 — 这是预期的，不是错误。
