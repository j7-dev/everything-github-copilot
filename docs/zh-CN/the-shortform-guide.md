...
**自 2 月实验性推出以来，我一直是 Copilot CLI 的忠实用户，并凭借 [zenith.chat](https://zenith.chat) 与 [@DRodriguezFX](https://x.com/DRodriguezFX) 一起赢得了 Anthropic x Forum Ventures 的黑客马拉松——完全使用 Copilot CLI。**
...
* **技能**: `~/.copilot/skills/` - 更广泛的工作流定义
* **命令**: `~/.copilot/commands/` - 快速可执行的提示词
...
*在 Copilot CLI 中运行 PostToolUse 钩子时获得的反馈示例*
...
**LSP 插件** 如果你经常在编辑器之外运行 Copilot CLI，则特别有用。语言服务器协议为 Claude 提供实时类型检查、跳转到定义和智能补全，而无需打开 IDE。
...
你的编辑器选择显著影响 Copilot CLI 的工作流。虽然 Copilot CLI 可以在任何终端中工作，但将其与功能强大的编辑器配对可以解锁实时文件跟踪、快速导航和集成命令执行。

### Zed（我的偏好）

我使用 [Zed](https://zed.dev) —— 用 Rust 编写，所以它真的很快。立即打开，轻松处理大型代码库，几乎不占用系统资源。

**为什么 Zed + Copilot CLI 是绝佳组合：**
...
1. **分割你的屏幕** - 一侧是带 Copilot CLI 的终端，另一侧是编辑器
...
这也是一个可行的选择，并且与 Copilot CLI 配合良好。你可以使用终端格式，通过 `\ide` 与你的编辑器自动同步以启用 LSP 功能（现在与插件有些冗余）。或者你可以选择扩展，它更集成于编辑器并具有匹配的 UI。

![VS Code Copilot CLI 扩展](../../assets/images/shortform/10-vscode-extension.jpeg)
*VS Code 扩展为 Copilot CLI 提供了原生图形界面，直接集成到您的 IDE 中。*
...
```markdown
ralph-wiggum@copilot-cli-plugins       # 循环自动化
frontend-design@copilot-cli-plugins    # UI/UX 模式
