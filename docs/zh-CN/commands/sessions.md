...
管理 Copilot CLI 会话历史 - 列出、加载、设置别名和编辑存储在 `~/.copilot/sessions/` 中的会话。
...
```bash
node -e "
const sm = require((process.env.PLUGIN_ROOT||require('path').join(require('os').homedir(),'.claude'))+'/scripts/lib/session-manager');
const aa = require((process.env.PLUGIN_ROOT||require('path').join(require('os').homedir(),'.claude'))+'/scripts/lib/session-aliases');
...
console.log('Path: ~/.copilot/sessions/' + session.filename);
console.log('');
...
* 会话以 Markdown 文件形式存储在 `~/.copilot/sessions/`
* 别名存储在 `~/.copilot/session-aliases.json`
