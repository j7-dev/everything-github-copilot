...
Bash({
  command: "~/.copilot/bin/codeagent-wrapper {{LITE_MODE_FLAG}}--backend <codex|gemini> {{GEMINI_MODEL_FLAG}}resume <SESSION_ID> - \"$PWD\" <<'EOF'
...
| 阶段 | Codex | Gemini |
|-------|-------|--------|
| 实施 | `~/.copilot/.ccg/prompts/codex/architect.md` | `~/.copilot/.ccg/prompts/gemini/frontend.md` |
| 审查 | `~/.copilot/.ccg/prompts/codex/reviewer.md` | `~/.copilot/.ccg/prompts/gemini/reviewer.md` |
...
1. 调用 Gemini（使用 `~/.copilot/.ccg/prompts/gemini/frontend.md`）
...
1. 调用 Codex（使用 `~/.copilot/.ccg/prompts/codex/architect.md`）
2. 输入：计划内容 + 检索到的上下文 + 目标文件
3. 输出：`Unified Diff Patch ONLY. Strictly prohibit any actual modifications.`
...
1. **Codex 审查**（`run_in_background: true`）：
   * ROLE\_FILE：`~/.copilot/.ccg/prompts/codex/reviewer.md`
   * 输入：更改的差异 + 目标文件
   * 重点：安全性、性能、错误处理、逻辑正确性

2. **Gemini 审查**（`run_in_background: true`）：
   * ROLE\_FILE：`~/.copilot/.ccg/prompts/gemini/reviewer.md`
   * 输入：更改的差异 + 目标文件
   * 重点：可访问性、设计一致性、用户体验
