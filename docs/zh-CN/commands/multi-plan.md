...
* **仅限规划**：此命令允许读取上下文并写入 `.copilot/plan/*` 计划文件，但 **绝不修改生产代码**
...
```
Bash({
  command: "~/.copilot/bin/codeagent-wrapper {{LITE_MODE_FLAG}}--backend <codex|gemini> {{GEMINI_MODEL_FLAG}}- \"$PWD\" <<'EOF'
...
| 阶段 | Codex | Gemini |
|-------|-------|--------|
| 分析 | `~/.copilot/.ccg/prompts/codex/analyzer.md` | `~/.copilot/.ccg/prompts/gemini/analyzer.md` |
| 规划 | `~/.copilot/.ccg/prompts/codex/architect.md` | `~/.copilot/.ccg/prompts/gemini/architect.md` |
...
1. **Codex 后端分析**：
   * ROLE\_FILE：`~/.copilot/.ccg/prompts/codex/analyzer.md`
   * 重点：技术可行性、架构影响、性能考虑、潜在风险
   * 输出：多视角解决方案 + 优缺点分析

2. **Gemini 前端分析**：
   * ROLE\_FILE：`~/.copilot/.ccg/prompts/gemini/analyzer.md`
...
1. **Codex 计划草案**（后端权威）：
   * ROLE\_FILE：`~/.copilot/.ccg/prompts/codex/architect.md`
   * 输出：分步计划 + 伪代码（重点：数据流/边缘情况/错误处理/测试策略）

2. **Gemini 计划草案**（前端权威）：
   * ROLE\_FILE：`~/.copilot/.ccg/prompts/gemini/architect.md`
...
2. 将计划保存到 `.copilot/plan/<feature-name>.md`（从需求中提取功能名称，例如 `user-auth`，`payment-module`）

3. 以 **粗体文本** 输出提示（必须使用实际保存的文件路径）：

   ***

   **计划已生成并保存至 `.copilot/plan/actual-feature-name.md`**
   ...
   * **执行计划**：复制以下命令到新会话
   ...
   ```
   /ccg:execute .copilot/plan/actual-feature-name.md
   ```
...
* **首次规划**：`.copilot/plan/<feature-name>.md`
* **迭代版本**：`.copilot/plan/<feature-name>-v2.md`，`.copilot/plan/<feature-name>-v3.md`...
...
1. 根据用户反馈调整计划内容
2. 更新 `.copilot/plan/<feature-name>.md` 文件
3. 重新呈现修改后的计划
4. 提示用户再次审阅或执行
...
```bash
/ccg:execute .copilot/plan/<feature-name>.md
```
