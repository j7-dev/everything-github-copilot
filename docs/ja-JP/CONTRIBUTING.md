# Everything Copilot CLIに貢献する

貢献いただきありがとうございます！このリポジトリはCopilot CLIユーザーのためのコミュニティリソースです。
...
```bash
# 1. Fork とクローン
gh repo fork affaan-m/everything-copilot-cli --clone
...
# 4. ローカルでテスト
cp -r skills/my-skill ~/.copilot/skills/  # スキルの場合
# その後、Copilot CLIでテスト
...
スキルは、コンテキストに基づいてCopilot CLIが読み込む知識モジュールです。
...
フックはCopilot CLIイベントによってトリガーされる自動的な動作です。
...
- **Issues:** [github.com/affaan-m/everything-copilot-cli/issues](https://github.com/affaan-m/everything-copilot-cli/issues)
