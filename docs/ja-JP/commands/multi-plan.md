...
- **計画のみ**: このコマンドはコンテキストの読み取りと`.copilot/plan/*`計画ファイルへの書き込みを許可しますが、**本番コードを変更しない**
...
2. 計画を`.copilot/plan/<feature-name>.md`に保存(要件から機能名を抽出、例: `user-auth`、`payment-module`)
3. **太字テキスト**でプロンプトを出力(**保存された実際のファイルパスを使用する必要があります**):

   ---
   **計画が生成され、`.copilot/plan/actual-feature-name.md`に保存されました**
...
- **最初の計画**: `.copilot/plan/<feature-name>.md`
- **反復バージョン**: `.copilot/plan/<feature-name>-v2.md`、`.copilot/plan/<feature-name>-v3.md`...
...
2. `.copilot/plan/<feature-name>.md`ファイルを更新
3. 変更された計画を再提示
