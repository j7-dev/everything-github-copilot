...
Copilot CLIセッション履歴を管理 - `~/.copilot/sessions/` に保存されたセッションのリスト表示、読み込み、エイリアス設定、編集を行います。
...
- セッションは `~/.copilot/sessions/` にMarkdownファイルとして保存されます
- エイリアスは `~/.copilot/session-aliases.json` に保存されます
