...
**Anthropicハッカソン優勝者による完全なCopilot CLI設定集。**
...
- **instinctインポート時のコンテンツ喪失を修正** — `/instinct-import`実行時に`parse_instinct_file()`がfrontmatter後のすべてのコンテンツ（Action、Evidence、Examplesセクション）を暗黙的に削除していた問題を修正。コミュニティ貢献者@ericcai0814により解決されました（[#148](https://github.com/affaan-m/everything-copilot-cli/issues/148), [#161](https://github.com/affaan-m/everything-copilot-cli/pull/161)）
...
完全なチェンジログは[Releases](https://github.com/affaan-m/everything-copilot-cli/releases)を参照してください。
...
> ⚠️ **重要:** Copilot CLIプラグインは`rules`を自動配布できません。手動でインストールしてください：
...
/everything-copilot-cli:plan "ユーザー認証を追加"
...
5. **グローバル設定**: `~/.copilot/package-manager.json`
...
または Copilot CLI で `/setup-pm` コマンドを使用。

---

## 📦 含まれるもの

このリポジトリは**Copilot CLIプラグイン**です - 直接インストールするか、コンポーネントを手動でコピーできます。
...
|
|-- rules/            # 常に従うべきガイドライン（~/.copilot/rules/ にコピー）
...
リポジトリから Copilot CLI スキルを生成する 2 つの方法：
...
両オプションで生成されるもの：
- **SKILL.mdファイル** - Copilot CLIですぐに使えるスキル
- **instinctコレクション** - continuous-learning-v2用
- **パターン抽出** - コミット履歴からの学習

### AgentShield — セキュリティ監査ツール

Copilot CLI 設定の脆弱性、誤設定、インジェクションリスクをスキャンします。
...
Copilot CLIで`/security-scan`を実行、または[GitHub Action](https://github.com/affaan-m/agentshield)でCIに追加できます。
...
### Copilot CLI CLI バージョン

**最小バージョン: v2.1.0 以上**

このプラグインは Copilot CLI CLI v2.1.0+ が必要です。プラグインシステムがフックを処理する方法が変更されたためです。
...
**背景:** これは本リポジトリで複数の修正/リバート循環を引き起こしました（[#29](https://github.com/affaan-m/everything-copilot-cli/issues/29), [#52](https://github.com/affaan-m/everything-copilot-cli/issues/52), [#103](https://github.com/affaan-m/everything-copilot-cli/issues/103)）。Copilot CLIバージョン間で動作が変わったため混乱がありました。今後を防ぐため回帰テストがあります。
...
このリポジトリを使用する最も簡単な方法 - Copilot CLIプラグインとしてインストール：
...
または、`~/.copilot/settings.json` に直接追加：
...
> **注:** Copilot CLIプラグインシステムは`rules`をプラグイン経由で配布できません（[アップストリーム制限](https://code.claude.com/docs/en/plugins-reference)）。ルールは手動でインストールする必要があります：
>
> ```bash
> # まずリポジトリをクローン
> git clone https://github.com/affaan-m/everything-copilot-cli.git
...
> cp -r everything-copilot-cli/rules/common/* ~/.copilot/rules/
> cp -r everything-copilot-cli/rules/typescript/* ~/.copilot/rules/   # スタックを選択
> cp -r everything-copilot-cli/rules/python/* ~/.copilot/rules/
> cp -r everything-copilot-cli/rules/golang/* ~/.copilot/rules/
>
...
`hooks/hooks.json` のフックを `~/.copilot/settings.json` にコピーします。
...
| コンポーネント | Copilot CLI → Cursor | パリティ |
|-----------|---------------------|--------|
...
| 機能 | Copilot CLI | OpenCode | ステータス |
|---------|-------------|----------|--------|
| Agents | ✅ 14 エージェント | ✅ 12 エージェント | **Copilot CLI がリード** |
| Commands | ✅ 30 コマンド | ✅ 24 コマンド | **Copilot CLI がリード** |
| Skills | ✅ 28 スキル | ✅ 16 スキル | **Copilot CLI がリード** |
...
OpenCodeのプラグインシステムはCopilot CLIより高度で、20+イベントタイプ：

| Copilot CLI フック | OpenCode プラグインイベント |
|-----------------|----------------------|
...
実験的なリリース以来、Copilot CLIを使用してきました。2025年9月、[@DRodriguezFX](https://x.com/DRodriguezFX)と一緒にCopilot CLIで[zenith.chat](https://zenith.chat)を構築し、Anthropic x Forum Venturesハッカソンで優勝しました。
...
- **簡潔ガイド（まずはこれ）:** [Everything Copilot CLI 簡潔ガイド](https://x.com/affaanmustafa/status/2012378465664745795)
- **詳細ガイド（高度）:** [Everything Copilot CLI 詳細ガイド](https://x.com/affaanmustafa/status/2014040193557471352)
