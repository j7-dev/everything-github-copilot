...
**來自 Anthropic 黑客松冠軍的完整 Copilot CLI 設定集合。**
...
<img src="https://github.com/user-attachments/assets/1a471488-59cc-425b-8345-5245c7efbcef" alt="Everything Copilot CLI 簡明指南" />
</a>
</td>
<td width="50%">
<a href="https://x.com/affaanmustafa/status/2014040193557471352">
<img src="https://github.com/user-attachments/assets/c9ca43bc-b149-427f-b551-af6840c368f0" alt="Everything Copilot CLI 完整指南" />
</a>
</td>
</tr>
<tr>
...
> ⚠️ **重要提示：** Copilot CLI 外掛程式無法自動分發 `rules`，需要手動安裝：
...
# 複製規則（應用於所有專案）
cp -r everything-copilot-cli/rules/* ~/.copilot/rules/
```
...
```bash
# 嘗試一個指令（外掛安裝使用命名空間形式）
/everything-copilot-cli:plan "新增使用者認證"
...
5. **全域設定**：`~/.copilot/package-manager.json`
6. **備援方案**：第一個可用的套件管理器
...
或在 Copilot CLI 中使用 `/setup-pm` 指令。

---

## 📦 內容概覽

本儲存庫是一個 **Copilot CLI 外掛程式** - 可直接安裝或手動複製元件。
...
|
|-- rules/            # 必須遵守的準則（複製到 ~/.copilot/rules/）
|   |-- security.md         # 強制性安全檢查
...
使用本儲存庫最簡單的方式 - 安裝為 Copilot CLI 外掛程式：
...
或直接新增到您的 `~/.copilot/settings.json`：
...
# 複製規則
cp everything-copilot-cli/rules/*.md ~/.copilot/rules/
...
將 `hooks/hooks.json` 中的鉤子複製到您的 `~/.copilot/settings.json`。
...
我從實驗性推出就開始使用 Copilot CLI。2025 年 9 月與 [@DRodriguezFX](https://x.com/DRodriguezFX) 一起使用 Copilot CLI 打造 [zenith.chat](https://zenith.chat)，贏得了 Anthropic x Forum Ventures 黑客松。
...
- **簡明指南（從這裡開始）：** [Everything Copilot CLI 簡明指南](https://x.com/affaanmustafa/status/2012378465664745795)
- **完整指南（進階）：** [Everything Copilot CLI 完整指南](https://x.com/affaanmustafa/status/2014040193557471352)
