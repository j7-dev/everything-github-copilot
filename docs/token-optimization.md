...
```json
{
  ...
  "env": {
    ...
    "COPILOT_AUTOCOMPACT_PCT_OVERRIDE": "50",
    "COPILOT_SUBAGENT_MODEL": "haiku"
  }
}
```
...
| `COPILOT_AUTOCOMPACT_PCT_OVERRIDE` | 95 | **50** | Auto-compaction triggers when context reaches this % of capacity. Default 95% is too late — quality degrades before that. Compacting at 50% keeps sessions healthier. |
| `COPILOT_SUBAGENT_MODEL` | _(inherits main)_ | **haiku** | Subagents (Task tool) run on this model. Haiku is ~80% cheaper and sufficient for exploration, file reading, and test running. |
...
- Enable with: `COPILOT_EXPERIMENTAL_AGENT_TEAMS=1` in settings
...
# Environment variables (add to ~/.copilot/settings.json "env" block)
MAX_THINKING_TOKENS=10000
COPILOT_AUTOCOMPACT_PCT_OVERRIDE=50
COPILOT_SUBAGENT_MODEL=haiku
